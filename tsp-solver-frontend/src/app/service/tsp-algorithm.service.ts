...
export class TspAlgorithmService {
  ...
  private readonly HILL_CLIMBING_URL = environment.backendUrl + '/api/tsp/hill-climbing';
  private readonly PARTICLE_SWARM_URL = environment.backendUrl + '/api/tsp/particle-swarm';
  private readonly COMBINES_ALGORITHM_1_URL = environment.backendUrl + '/api/tsp/combined-algorithm-1';
  ...
  public getHillClimbing(costMatrix: number[][]): Observable<RouteAlgorithm> {
    return this.httpClient.post<RouteAlgorithm>(this.HILL_CLIMBING_URL, costMatrix);
  }

  public getParticleSwarm(costMatrix: number[][]): Observable<RouteAlgorithm> {
    return this.httpClient.post<RouteAlgorithm>(this.PARTICLE_SWARM_URL, costMatrix);
  }

  public getCombinedAlgorithm1(request: GeneticAlgorithmRequest): Observable<RouteAlgorithm> {
    return this.httpClient.post<RouteAlgorithm>(this.COMBINES_ALGORITHM_1_URL, request);
  }
