...
export class AlgorithmFormComponent implements OnInit {
  ...
  geneticSAAlgorithmForm = this.fb.group({
    populationSize: [100, [Validators.required, Validators.min(1), Validators.pattern("^[0-9]*$")]],
    elitismSize: [20, [Validators.required, Validators.min(0), Validators.pattern("^[0-9]*$")]],
    mutationRate: [0.01, [Validators.required, Validators.min(0.0), Validators.max(1)]],
    epochs: [3000, [Validators.required, Validators.min(1), Validators.pattern("^[0-9]*$")]],
  ...
  ngOnInit(): void {
    ...
    this.geneticSAAlgorithmForm.setValidators(this.greaterThan('populationSize', 'elitismSize'));

    this.geneticSAAlgorithmForm.get('selectionType')!.setValue(this.selectionTypes.length > 0 ? this.selectionTypes[0] : '');
    this.geneticSAAlgorithmForm.get('crossoverType')!.setValue(this.crossoverTypes.length > 0 ? this.crossoverTypes[0] : '');
  }
  ...
  runAlgorithm() {
    ...
    switch (this.chosenAlgorithm) {
      ...
      case '5': {
        routeAlgorithmRow.algorithmType = 'Hill climbing';
        routeAlgorithm = this.tspAlgorithmService.getHillClimbing(this.costMatrix);
        break;
      }
      case '6': {
        routeAlgorithmRow.algorithmType = 'Particle swarm optimization';
        routeAlgorithm = this.tspAlgorithmService.getParticleSwarm(this.costMatrix);
        break;
      }
      case '7': {
        ...
        routeAlgorithmRow.algorithmType = 'Genetic algorithm + Simulated annealing';
        routeAlgorithmRow.parametersTranslation = 'GeneticAlgorithmParameters';
        routeAlgorithmRow.parameters = {
          populationSize: populationSize,
          elitismSize: elitismSize,
        ...
        const geneticAlgorithmRequest = new GeneticAlgorithmRequest(this.costMatrix, populationSize,
          elitismSize, mutationRate, epochs, maxEpochsNoImprovement, selectionType, crossoverType);
        routeAlgorithm = this.tspAlgorithmService.getCombinedAlgorithm1(geneticAlgorithmRequest);
        break;
      }
