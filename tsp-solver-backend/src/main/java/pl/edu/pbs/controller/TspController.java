...
public class TspController {
    ...
    public ResponseEntity<Route> getSimulatedAnnealing(
            @RequestBody SimulatedAnnealingRequest simulatedAnnealingRequest) {
    ...
    @PostMapping(path = "/hill-climbing")
    public ResponseEntity<Route> getHillClimbing(@RequestBody double[][] costMatrix) {
        Route route = tspService.getHillClimbing(costMatrix);
        return new ResponseEntity<>(route, HttpStatus.OK);
    }

    @PostMapping(path = "/particle-swarm")
    public ResponseEntity<Route> getParticleSwarm(@RequestBody double[][] costMatrix) {
        Route route = tspService.getParticleSwarm(costMatrix);
        return new ResponseEntity<>(route, HttpStatus.OK);
    }

    @PostMapping(path = "/combined-algorithm-1")
    public ResponseEntity<Route> getGeneticSimulatedAlgorithm(
            @RequestBody GeneticAlgorithmRequest geneticAlgorithmRequest) {
        Route route = tspService.getGeneticSAAlgorithm(geneticAlgorithmRequest);
        return new ResponseEntity<>(route, HttpStatus.OK);
    }
