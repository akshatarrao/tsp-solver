...
public class TwoOpt extends Algorithm {
    ...
    public TwoOpt(ArrayList<Integer> bestRoute) {
    ...
    protected Route solve(double[][] costMatrix) {
        if (bestRoute == null || bestRoute.size() != costMatrix.length + 1) {
            bestRoute = (ArrayList<Integer>) generateRandomRoute(costMatrix.length);
        }
            ...
            for (int i = 1; i < costMatrix.length - 1; i++) {
                for (int j = i + 1; j < costMatrix.length; j++) {
                    ...
                    if (isMatrixSymmetric) {
                        deltaCost = costMatrix[newRoute.get(i - 1)][newRoute.get(i)]
                                + costMatrix[newRoute.get(j)][newRoute.get(j + 1)]
                                - costMatrix[bestRoute.get(i - 1)][bestRoute.get(i)]
                                - costMatrix[bestRoute.get(j)][bestRoute.get(j + 1)];
                    } else {
